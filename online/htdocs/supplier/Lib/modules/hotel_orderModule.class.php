...
class hotel_orderModule extends AuthModule
{
	...
	function index() {
		...
		$totalCount = $GLOBALS['db']->getOne("select count(id) from ".DB_PREFIX."hotel_room_order t where ".$condition);
		if($totalCount){
			$sql = "select t.*,u.user_name,u.mobile,s.user_name as supplier_name  from ".DB_PREFIX."hotel_room_order t left outer join ".DB_PREFIX."user u on u.id = t.user_id left outer join ".DB_PREFIX."supplier s on s.id = t.supplier_id where ".$condition."  order by ".$param['orderField']." ".$param["orderDirection"]." limit ".$limit;
			//echo $sql;
			//die();
		...
		*/
		...
		$GLOBALS['tmpl']->assign("formaction",admin_url("hotel_order"));
		$GLOBALS['tmpl']->assign("editurl",admin_url("hotel_order#order"));
		$GLOBALS['tmpl']->assign("delurl",admin_url("hotel_order#del_order",array("ajax"=>1)));
		$GLOBALS['tmpl']->assign("exporturl",admin_url("hotel_order#export_csv"));
		//$GLOBALS['tmpl']->display("core/spot_order/index.html");
		$GLOBALS['tmpl']->display("core/hotel_order/index.html");
	}
	...
	function export_csv($page = 1) {
		...
		header("Content-Disposition: attachment; filename=hotel_order.csv");
		echo $content;		
	...
	public function order()
	{
		...
		$sql = "select t.*,u.user_name,u.mobile,s.user_name as supplier_name  from ".DB_PREFIX."hotel_room_order t left outer join ".DB_PREFIX."user u on u.id = t.user_id left outer join ".DB_PREFIX."supplier s on s.id = t.supplier_id where  t.id = ".$id;
		...
		//门票列表;
		$ticketlist = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."hotel_room_order_item where order_id = ".intval($id));
		...
		//订单日志;
		$order_log = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."hotel_room_order_log where order_id = ".intval($id)."  order by log_time");
		...
		$GLOBALS['tmpl']->assign("pay_order_url",admin_url("hotel_order#pay_order",array("ajax"=>1,id=>$id)));
		$GLOBALS['tmpl']->assign("order_status_url",admin_url("hotel_order#do_order_status",array("ajax"=>1,id=>$id)));
		$GLOBALS['tmpl']->assign("refund_status_url",admin_url("hotel_order#do_refund_status",array("ajax"=>1,id=>$id)));
		$GLOBALS['tmpl']->assign("use_verify_code_url",admin_url("hotel_order#use_verify_code",array("ajax"=>1,id=>$id,'verify_code'=>$order['verify_code'])));
		 
		$GLOBALS['tmpl']->assign("refuse_refund_url",admin_url("hotel_order#refuse_refund",array("ajax"=>1,id=>$id)));
		 
		
		$GLOBALS['tmpl']->assign("order_item_url",admin_url("hotel_order#order_item",array("ajax"=>1,'order_id'=>$id)));
		
		//发货
		$GLOBALS['tmpl']->assign("order_delivery_url",admin_url("hotel_order#delivery",array("ajax"=>1,'id'=>$id)));
		//标识用户收货
		$GLOBALS['tmpl']->assign("order_delivery2_url",admin_url("hotel_order#do_delivery",array("ajax"=>1,id=>$id)));
		
		$GLOBALS['tmpl']->assign("sendurl",admin_url("hotel_order#send_sms_mail",array("ajax"=>1,'id'=>$id)));
		 		
		$GLOBALS['tmpl']->display("core/hotel_order/order.html");
	}
	...
	public function order_item()
	{
		...
		$GLOBALS['tmpl']->assign("formaction",admin_url("hotel_order#do_re_appoint_status",array("ajax"=>1,"order_id"=>$item['order_id'],id=>$id)));
		$GLOBALS['tmpl']->assign("accounturl",admin_url("hotel_order#order",array("ajax"=>1,id=>$id)));
				
		$GLOBALS['tmpl']->display("core/hotel_order/order_item.html");
	}
	...
	public function delivery()
	{
		...
		$sql = "select t.* from ".DB_PREFIX."hotel_order t where t.id = ".$id;
		...
		$GLOBALS['tmpl']->assign("formaction",admin_url("hotel_order#do_delivery",array("ajax"=>1,id=>$id)));
		$GLOBALS['tmpl']->assign("accounturl",admin_url("hotel_order#order",array("ajax"=>1,id=>$id)));
	
		$GLOBALS['tmpl']->display("core/hotel_order/delivery.html");
	}	
	...
	public function do_delivery()
	{
		...
		$id = intval($_REQUEST['id']);
		$order = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_order where id = ".$id);
		if(empty($order))
		{
			showErr("订单不存在",$ajax,admin_url("hotel_order#order",array(id=>$id)))	;
		}else{
			require_once APP_ROOT_PATH."system/libs/hotel.php";
			...
			if ($order['delivery_status'] == 0){
				...
				$GLOBALS['db']->autoExecute(DB_PREFIX."hotel_room_order",$data,"UPDATE","id=".$id,"SILENT");
				
				save_ticket_order_log($id,'订单发货:'.$data['delivery_sn'],2);
								
				$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order where id = '".$id."'");
				//订单发货 短信通知
				...
				showSuccess('发货成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
				...
				showSuccess('订单确认收货成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}	
		}
	}
	...
	public function do_re_appoint_status()
	{
		...
		$order_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order_item where re_appoint_status = 1 and id = ".$id);
		if(empty($order_item))
		{
			showErr("用户未申请改约时间或已被处理",$ajax,admin_url("hotel_order#order",array(id=>$order_id)));
		}
		...
		if ($re_appoint_status == 2){
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order_item set begin_time = $begin_time,end_time = $end_time, appoint_time = re_action_time, re_appoint_status = 2,re_appoint_refuse_reason='".$re_appoint_refuse_reason."' where re_appoint_status = 1 and id = ".$id." ","SILENT");
		
			 
			if($GLOBALS['db']->affected_rows()>0){
				
				$i = intval($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."hotel_room_order_item where re_appoint_status = 1 and order_id = ".$order_id." ","SILENT"));
				if ($i == 0){
					//`re_appoint_status` tinyint(1) NOT NULL COMMENT '是否有门票改签申请，0无 1有',
					$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set re_appoint_status = 0 where id = ".$order_id." ","SILENT");					
				}
				
				
				save_ticket_order_log($order_id,$id.':改期:'.to_date($order_item['re_action_time'],"Y-m-d"),2);
				
				$order = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order where id = ".$order_id);
				...
				showSuccess('改期成功',$ajax,admin_url("hotel_order#order",array(id=>$order_id)));
			}else{
				showSuccess('改期失败',$ajax,admin_url("hotel_order#order",array(id=>$order_id)));
			}
		}else if ($re_appoint_status == 3){
	
			$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set re_appoint_status = 3,re_appoint_refuse_reason='".$re_appoint_refuse_reason."' where re_appoint_status = 1 and id = ".$id." ","SILENT");
			if($GLOBALS['db']->affected_rows()>0){
				...
				showSuccess('拒绝成功',$ajax,admin_url("hotel_order#order",array(id=>$order_id)));
			}else{
				save_tourline_order_log($order_id,'管理员后台拒绝退款失败',1);
	
				showSuccess('拒绝失败',$ajax,admin_url("hotel_order#order",array(id=>$order_id)));
			}
		}
	}
	...
	public function del_order()
	{
		...
		if (isset ( $_REQUEST ['id'] ))
		{
			...
			if($id)
			{
				$sql = "select id,sn from ".DB_PREFIX."hotel_room_order where order_status = 1 and id in (".$id.")";
				$list = $GLOBALS['db']->getAll($sql);
				$id = "";
				$order_sn = "";
				foreach($list as $k=>$v)
				...
				if ($id != ""){
					$sql = "delete from ".DB_PREFIX."hotel_room_order where order_status = 1 and id in (".$id.")";
					...
					if($GLOBALS['db']->affected_rows()>0)
					{
						...
						$sql = "delete from ".DB_PREFIX."hotel_room_order_item where order_id in (".$id.")";
						$GLOBALS['db']->query($sql);
						 
						$sql = "delete from ".DB_PREFIX."hotel_room_order_log where order_id in (".$id.")";
						$GLOBALS['db']->query($sql);
	...
	public function pay_order()
	{
		$id = intval($_REQUEST['id']);
		$ajax = intval($_REQUEST['ajax']);
		$order = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order where pay_status = 0 and id = ".$id);
		if(empty($order))
		{
			showErr("订单不存在或已收款",$ajax,admin_url("hotel_order#order",array(id=>$id)))	;
		}
		//1、生成一张收款单
		//2、完成收款
		//3、订单状态变成：已确认
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set order_status = 2,confirm_time = '".NOW_TIME."' where order_status = 1 and id = ".$id." ","SILENT");
	
		save_ticket_order_log($id,'管理员后台订单收款:'.format_price(format_price_to_display($money)),2);
		 
		showSuccess('收款成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
	}
	...
	public function do_order_status()
	{
		...
		if ($order_status == 2 || $order_status == 5){
			...
			showSuccess('确认订单成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
		}else if ($order_status == 3){
			//完成订单
			if (ticket_order_complete($id,2)){
				showSuccess('完成订单成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}else{
				showErr("不是已经确认的订单,不能直接完成",$ajax,admin_url("hotel_order#order",array(id=>$id)))	;
			}
		}else if ($order_status == 4){
			//订单作废
			if (ticket_order_invalid($id,2)){
				showSuccess('完成订单作废',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}else{
				showErr("订单不存在或已被作废",$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}
		}
	}
	...
	public function refuse_refund()
	{
		...
		$ajax = intval($_REQUEST['ajax']);
		//refuse_reason
		$order = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order where refund_status = 1 and id = ".$id);
		...
		require_once APP_ROOT_PATH."system/libs/hotel.php";
		//门票列表;
		$ticketlist = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."hotel_room_order_item where refund_status = 1 and order_id = ".intval($id));
		foreach($ticketlist as $k=>$v)
		{
			ticket_order_item_format($ticketlist[$k]);
		}
	...
	//	print_r($order);
		...
		$GLOBALS['tmpl']->assign("formaction",admin_url("hotel_order#do_refund_status",array("ajax"=>1,id=>$id)));
		$GLOBALS['tmpl']->assign("accounturl",admin_url("hotel_order#order",array("ajax"=>1,id=>$id)));
		//$GLOBALS['tmpl']->display("core/user/op_account.html");
		$GLOBALS['tmpl']->display("core/hotel_order/refuse_refund.html");
	...
	public function do_refund_status()
	{
		...
		if(empty($order))
		{
			showErr("用户未申请退款或退款单已被处理:".$id,$ajax,admin_url("hotel_order#order",array(id=>$id)))	;
		}
		...
		if ($refund_status == 2){
			...
			if($res['return']){
				showSuccess('退款成功',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}else{
				showSuccess($res['message'],$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}
		}else if ($refund_status == 3){
			

			$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set refund_status = 3,refuse_reason='".$refuse_reason."' where refund_status = 1 and id = ".$id." ","SILENT");
			if($GLOBALS['db']->affected_rows()>0){
				if ($order['is_divide'] == 1){
					$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order_item set refund_status = 3 where order_id = ".$id." ","SILENT");
				}
				...
				showSuccess('管理员后台拒绝退款',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}else{
				save_ticket_order_log($id,'管理员后台拒绝退款失败',2);
	
				showSuccess('管理员后台拒绝退款失败',$ajax,admin_url("hotel_order#order",array(id=>$id)));
			}
		}
