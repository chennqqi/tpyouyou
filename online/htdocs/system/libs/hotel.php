 ...
 * $citys 城市拼音 多个,隔开
 */
function get_hotel_list($citys="",$condtions="",$order="",$limit="0,10"){
	$condition = " 1 = 1 ";
	...
	$result["rs_count"] = $GLOBALS['db']->getOne('SELECT count(*) FROM '.DB_PREFIX.'hotel WHERE '.$condition);
	if($result["rs_count"] > 0){
		$result["list"] = $GLOBALS['db']->getAll('SELECT * FROM '.DB_PREFIX.'hotel WHERE '.$condition.' ORDER BY '.$order.' LIMIT '.$limit);
...
function get_spot($id){
	$spot = $GLOBALS['db']->getRow('SELECT * FROM '.DB_PREFIX.'hotel WHERE id='.intval($id));
...
function get_supplier_spot($sid){
	$spot = $GLOBALS['db']->getRow('SELECT * FROM '.DB_PREFIX.'hotel_supplier WHERE id='.intval($sid));
	if($spot){
		...
		$spot['ticket_list'] = unserialize($spot['room_list']);
		$spot['tickets'] = array();
...
function get_ticket($id){
	if($id >0)
		return $GLOBALS['db']->getRow('SELECT * FROM '.DB_PREFIX.'hotel_room WHERE is_effect=1 and id= '.intval($id));
	else
		return false;
}
...
function format_spot($spot){
	...
	$room_list = $GLOBALS['db']->getAll('SELECT * FROM '.DB_PREFIX.'hotel_room WHERE hotel_id='.$spot['id']);
...
function bulid_spot_url($spot_id){
	return url("hotel#view",array("id"=>$spot_id));
}
...
function save_ticket_order_log($order_id,$log_info,$is_supplier)
{
	...
	$GLOBALS['db']->autoExecute(DB_PREFIX."hotel_room_order_log",$log_data);
}
...
function ticket_order_info($order_sn){
	...
	if(count($order_sn_e)==1){
		$order_data = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."hotel_room_order WHERE sn = '".$order_sn."' ");
		if($order_data){
			$order_data['money'] = $order_data['total_price'] - $order_data['pay_amount'];
		}
	}
	else{
		$order_data = $GLOBALS['db']->getRow("SELECT sum(total_price) as a_total_price,sum(pay_amount) as a_pay_amount,min(end_time) as end_time  FROM ".DB_PREFIX."hotel_room_order WHERE sn in ('".implode("','",$order_sn_e)."') ");
		if($order_data){
			...
			$order_data['order_list'] = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."hotel_room_order WHERE sn in ('".implode("','",$order_sn_e)."') ");
		}
	}
...
function ticket_order_paid($order_sn,$online_pay=0,$account_pay=0,$voucher_pay =0){
		...
		$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_room_order where sn = '".$order_sn."'");
		$id = $order_info['id'];
		...
		$sql = "update ".DB_PREFIX."hotel_room_order set ".
		...
		if($order_info['order_confirm_type'] ==2)
			$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set pay_status = 1,order_status = 2,confirm_time = '".NOW_TIME."' where pay_status = 0 and pay_amount >= total_price and id = ".$id);
		else
			$GLOBALS['db']->query("update ".DB_PREFIX."hotel_room_order set pay_status = 1 where pay_status = 0 and pay_amount>=total_price and id = ".$id);
			
		if($GLOBALS['db']->affected_rows()>0)
		{
			//支付成功 添加实际销售量
			$hotel_id = intval($order_info['hotel_id']);
			$sale_count = intval($order_info['sale_count']);
			$GLOBALS['db']->query("update ".DB_PREFIX."hotel set sale_total = sale_total + ".$sale_count." where id = ".$hotel_id);
			
			$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."hotel_order_room where sn = '".$order_sn."'");
			//订单支付成功，发短信
